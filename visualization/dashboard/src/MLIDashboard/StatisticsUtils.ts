...
export const generateMetrics: (jointDataset: JointDataset, selectionIndexes: number[][], modelType: ModelTypes) => ILabeledStatistic[][] = 
    (jointDataset: JointDataset, selectionIndexes: number[][], modelType: ModelTypes): ILabeledStatistic[][] => {
...
const generateRegressionStats: (trueYs: number[], predYs: number[], errors: number[]) => ILabeledStatistic[] =
    ...
    return [
        ...
        },
        {
            ...
            stat: predYs.reduce((prev, curr) => {return prev + curr}, 0)/ count
        }
    ];
};
